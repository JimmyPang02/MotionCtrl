...
from main.evaluation.motionctrl_prompts_camerapose_trajs_webvid10 import (
...
def load_trajs(cond_dir, trajs):
    ...
    # 处理trajs数组，生成traj_name数组
    def process_trajs(trajs_files):
        traj_names = []
        print(traj_files)

        for i,traj_path in enumerate(trajs_files):
            # 提取page_dir和videoid_dir
            parts = traj_path.split('/')
            page_dir = parts[-3]
            videoid_dir = parts[-2]
            
            # 拼接成traj_name
            # traj_name = f"{page_dir}_{videoid_dir}"  # OSError: [Errno 36] File name too long
            traj_name =  videoid_dir.split('_')[0] + f"{i}" # 再次简化
            traj_names.append(traj_name)

        return traj_names
    traj_name=process_trajs(traj_files)
    print(traj_name)
    ...
    for idx in range(total_files):
        print(f"Loading trajectory {idx + 1}/{total_files}...")
        traj_data = np.load(traj_files[idx])
        ...
        for frame in traj_data:
            ...
            # 调整大小为256x256
            resized_frame = cv2.resize(frame, (256, 256), interpolation=cv2.INTER_NEAREST)
            resized_traj_data.append(resized_frame)
        
        resized_traj_data = np.array(resized_traj_data)
        print(resized_traj_data.shape)
        
        # 转换为torch tensor并调整维度
        data_list.append(torch.tensor(resized_traj_data).permute(3, 0, 1, 2).float()) # [t,h,w,c] -> [c,t,h,w]
...
def run_inference(args, gpu_num, gpu_no):
    ...
    for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
        ...
        for nn, samples in enumerate(batch_samples):
            ...
            prompt = prompts[nn]
            name = save_name[nn]
            if len(name) > 35: # 之前是90，会出错，OSError: [Errno 36] File name too long
                name = name[:35]
            filename = f'{name}_{idx*args.bs+nn:04d}_randk{gpu_no}'
            
            save_results(samples, filename, savedir, fps=10)
            if args.save_imgs:
                if len(save_name[nn]) > 35: # 保存帧的时候也要判断，不然会出错
                    save_name[nn] = save_name[nn][:35]                
