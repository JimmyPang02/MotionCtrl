...
def get_RT(pose_dir='', video_frames=14, frame_stride=1, speed=1.0, **kwargs):
    ...
    #pose_dir 分割取出文件名作为 pose_name
    pose_name = [pose_file[0].split('/')[-1].split('.')[0]] 
...
def run_motionctrl_inference(
    seed: int = 12345,
    ckpt: str = "../../model/motionctrl/motionctrl_svd.ckpt",
    config: str = "configs/inference/config_motionctrl_cmcm.yaml",
    savedir: str = "outputs/motionctrl_svd",
    savefps: int = 10,
    ddim_steps: int = 25,
    frames: int = 14,
    image_input: str = "examples/basic/eduardo-gorghetto-5auIBbcoRNw-unsplash.jpg",
    fps: int = 10,
    motion: int = 127,
    cond_aug: float = 0.02,
    decoding_t: int = 1,
    ...
    sample_num: int = 2,
    transform: bool = True,
    pose_dir: str = "examples/camera_poses_evaluate",
    speed: float = 2.0,
    save_images: bool = False,
    device: str = "cuda",
):
    """
    将原先脚本的命令行调用，改成一个函数调用。
    """
